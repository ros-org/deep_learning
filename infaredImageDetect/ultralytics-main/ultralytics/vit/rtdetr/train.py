# 注意：官方很多default.yaml中的参数未使用，如batch_size、imgsz需要在当前脚本70行args中设置。但是epoch等参数用到了。
#       调用哪个规模的模型在69行设置,rtdetr-l.yaml, rtdetr-x.yaml, yolov8-rtdetr.yaml均可以；
...
# 用下面这行代替上面那行，上面那行按理是一样的效果，不知道为什么有时会导致sam推理报错；
from ultralytics.vit.rtdetr.val import RTDETRDataset, RTDETRValidator
...
def train(cfg=DEFAULT_CFG, use_python=False):
    ...
    args = dict(model=model,
                data=data,
                device=device,
                # imgsz=640,    # 注销该行，改用default.yaml文件中的imgsz参数
                exist_ok=True,
                # batch=2,      # 注销该行，改用default.yaml文件中的batch参数
                deterministic=False,
                amp=False)
