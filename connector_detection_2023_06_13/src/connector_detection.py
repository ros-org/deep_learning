...
def train_on_solar_panel(configurations=None,
    ...
    model_path = configurations.get(
        'model_path', r'/media/drin/shared_disk/work/cv/2023_06_07_connector/pretrained_models/yolov8x.pt')
    model_path = pathlib.Path(model_path).expanduser().resolve()
    print(f'{model_path= }')

    target_model_path = r'/media/drin/shared_disk/work/cv/2023_06_07_connector/pretrained_models'
    target_model_path = pathlib.Path(target_model_path).expanduser().resolve()

    detect_data = configurations.get(
        'detect_data', r'/media/drin/shared_disk/work/cv/2023_06_07_connector/tryout/src/connector.yaml')
    ...
    experiment_name = f'{model_id}_{optimizer}_lr{lr0:.2e}_lrf{lrf:.2f}_conf{conf}_iou{iou}' \
                      f'_b{batch}_e{epochs}_nbs{nbs}'  # \_deg{degrees}
...
if __name__ == '__main__':
    configurations = {'model_id': r'dx015',
                      # 'model_path': None,
                      ...
                      # 'nbs': 16,
                      'optimizer': 'Adam',
                      'lr0': 5.95e-5,  # 'lr0': sgd--3.87e-3, 5e-6  5.95e-5
                      # 'lrf': 1,  # 'lrf': 1
                      'epochs': 1300}  # 220
    train_on_solar_panel(configurations=configurations)  # 训练模型

    # 用训练好的模型对测试集图片进行预测 dx003_Adam_lr1.00e-05_b4_e10_nbs16_val933_test940
    # model_path = r'/media/drin/shared_disk/work/cv/2023_06_07_connector/pretrained_models/' \
    #              r'dx007_Adam_lr5.95e-06_lrf0.01_conf0.5_iou0.7_b4_e220_nbs4_val920_test891.pt'  # noqa
    # tryout_images = r'/media/drin/shared_disk/work/cv/2023_06_07_connector/dataset_connector/images/test'
