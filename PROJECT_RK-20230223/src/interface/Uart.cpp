...
Uart::~Uart()
{
    ...
    // 销毁一个互斥锁即释放它所占用的资源，且要求锁当前处于开放状态。在Linux中，互斥锁并不占用任何资源,因此LinuxThreads中的pthread_mutex_destroy()除了检查锁状态以外(锁定状态则返回EBUSY)没有其他动作
...
// returnValue:None
// ----------------------------------->parameters<----------------------------------
void Uart::getMsgFromMainThread(INPUT unsigned char& signalValue)
{
    pthread_mutex_lock(&m_uartMutex2);
    m_messageFromMainThread = signalValue;
...
int Uart::run()
{
    ...
    while (true)  
    {
        ...
        if (0 == m_waitFlag)  
        {
            ...
            m_waitFlag = -999;               //读完发来的消息就置为-999,等待下次变为0
        } 

        //发消息，从另一个线程获取到图像算法结果，根据结果值发消息给驱动板
        if(0 <= m_messageFromMainThread && m_messageFromMainThread <= 5)
        {
            pthread_mutex_lock(&m_uartMutex2);
            
            send_to_stm32(&m_messageFromMainThread, 1);
            i++;
            m_messageFromMainThread = 255;
            pthread_mutex_unlock(&m_uartMutex2);
        }
...
{   
    ...
    m_bufferSend[m_len++] = 0x06;              // 固定值
    m_bufferSend[m_len++] = 0x08;              // 存放地址
    m_bufferSend[m_len++] = 0x1f;              // 存放地址
