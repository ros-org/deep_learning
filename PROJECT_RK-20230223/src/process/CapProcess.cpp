...
int CapProcess::init()
{
    m_start = false;                                                           //当是空图时为false则预测线程也不会取图成功，同时还可以保证每一张图至多倍预测线程预测一次;
    m_bdebug = true;                                                     //控制图片来自本地还是摄像头(true:加载硬盘上的图片检测; false:从摄像头获取图片进行检测) 
    m_interval = 2;                                                            //帧间隔，摄像头隔m_interval帧向缓存中放图
...
int CapProcess::start()
{
    ...
    if (m_bdebug == true) 
    {
        std::cout<<"Read local image..."<<std::endl;
        Mat im_test = cv::imread("/userdata/data/test_1.jpg"); 
        std::cout<<"原始图像通道数="<<im_test.channels()<<std::endl;
