	             ...
	             OUTPUT cv::Mat& dst)
{
	...
	std::cout << "selectShape 00000000000000" << std::endl;
	std::vector<std::vector<cv::Point>> contours;

	cv::findContours(src, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE);
	std::cout << "selectShape 22222222222" << std::endl;
	int cnum = contours.size();

	std::cout << "selectShape 333333333333333" << std::endl;
...
LT::ErrorValue postProcessingSegRes(INPUT cv::Mat& image, OUTPUT float& angle)
{
	...
	int value = image.at<uchar>(39, 277);
	cv::Mat thresImage;
	cv::threshold(image, thresImage, 1.5, 255, 3);

	cout << "postProcessingSegRes 11111111111111111" << endl;
	...
	cout << "postProcessingSegRes 2222222222222" << endl;
	selectShape(thresImage, selectType, operation, mins, maxs, selectedImage);
	cout << "postProcessingSegRes 33333333333333" << endl;
	cv::Mat eroImage;
	...
	{
		// Do something
		cout << "postProcessingSegRes error  1111111111111111 " << endl;
