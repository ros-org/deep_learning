...
class Uart      // 定义通信类
{
public:         // 定义多个public / private  有时是为了区分属性的public和方法的public
       
private:
    // bool m_binit;
    // struct termios m_options;
    int m_fd;                                                                     //打开的串口文件句柄
    //unsigned char m_bufferSend[8];                                              //发送消息的buffer(缓存)，存储发送给开发板的数据
    unsigned char m_bufferSend[13];                                               //发送消息的buffer(缓存)，存储发送给开发板的数据
    ...
    pthread_t m_uartTid;                                                          //线程标识  表示线程 id ,                                       
    char* m_portName;                                                             //串口端口号，字符串，"/dev/ttyS3"：代表端口号3;
    unsigned char m_mesageFromDriverCard;                                         //收到的从驱动板发来的具体消息值
    unsigned char m_messageFromMainThread[4];                                     //从主线程获取到的消息，消息线程会将该消息发给驱动板
...
public:
    ...
    void getMsgFromMainThread(INPUT unsigned char* signalValues);                 //线程数据交互函数，将消息从主线程发到消息线程   
